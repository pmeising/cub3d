...
/*   Updated: 2023/01/14 17:18:59 by pmeising         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_resize_tex_south(t_prgrm *vars, t_img *img, int x, int y)
{
	...
	color = *(int *)(wall->addy_img + ((int)(vars->ray->texY) % wall->height[0] * wall->line_length + vars->ray->texX % 64 * (wall->bits_per_pixel / 8)));
...
void	ft_resize_door(t_prgrm *vars, t_img *img, int x, int y)
{
	...
	color = *(int *)(wall->addy_img + ((int)(vars->ray->texY) % wall->height[0] * wall->line_length + vars->ray->texX % wall->width[0] * (wall->bits_per_pixel / 8)));
	my_mlx_pixel_put(img, x, y, color);
}
